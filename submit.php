    ...
    if (isset($_SESSION['cv'])) {
        ...
        if (isset($_SESSION['organisasi']) && !empty($_SESSION['organisasi'])) {
            foreach ($_SESSION['organisasi'] as $organisasi) {
                $stmt = $pdo->prepare("INSERT INTO tb_organisasi (cv_id, nama_organisasi, jabatan_organisasi, tahun_organisasi) VALUES (?, ?, ?, ?)");
        ...
        if (isset($_SESSION['pelatihan']) && !empty($_SESSION['pelatihan'])) {
            foreach ($_SESSION['pelatihan'] as $pelatihan) {
                ...
                $stmt = $pdo->prepare("INSERT INTO tb_pelatihan (id_cv, nama_pelatihan, lembaga_pelatihan, tahun_pelatihan, dokumen_pelatihan) VALUES (?, ?, ?, ?, ?)");
        ...
        if (isset($_SESSION['pendidikan']) && !empty($_SESSION['pendidikan'])) {
            foreach ($_SESSION['pendidikan'] as $pendidikan) {
                ...
                $stmt = $pdo->prepare("INSERT INTO tb_pendidikan (id_cv, nama_pendidikan, jurusan_pendidikan, tahun_lulus, dokumen_pendidikan) VALUES (?, ?, ?, ?, ?)");
        ...
        if (isset($_SESSION['pengalaman']) && !empty($_SESSION['pengalaman'])) {
            foreach ($_SESSION['pengalaman'] as $pengalaman) {
                ...
                $stmt = $pdo->prepare("INSERT INTO tb_pengalaman (id_cv, tahun_mulai_pengalaman, tahun_selesai_pengalaman, tempat_pengalaman, deskripsi_pengalaman) VALUES (?, ?, ?, ?, ?)");
        ...
        if (isset($_SESSION['sertifikasi']) && !empty($_SESSION['sertifikasi'])) {
            foreach ($_SESSION['sertifikasi'] as $sertifikasi) {
                ...
                $stmt = $pdo->prepare("INSERT INTO tb_sertifikasi (id_cv, nama_sertifikasi, lembaga_sertifikasi, tahun_sertifikasi, dokumen_sertifikasi) VALUES (?, ?, ?, ?, ?)");
        ...
        if (isset($_SESSION['skill']) && !empty($_SESSION['skill'])) {
            foreach ($_SESSION['skill'] as $skill) {
                ...
                $stmt = $pdo->prepare("INSERT INTO tb_skill (id_cv, nama_skill) VALUES (?, ?)");
                $stmt->execute([$cvId, $skill['nama_skill']]);
            }
        }
        if (isset($_SESSION['media']) && !empty($_SESSION['media'])) {
            foreach ($_SESSION['media'] as $media) {
                $stmt = $pdo->prepare("INSERT INTO tb_sosial_media (id_cv, jenis_sosial_media, nama_sosial_media) VALUES (?, ?, ?)");
